...
// grab the contributions card elementasz
...
function filterUnfundedOnly() {
    ...
    const listofGames = GAMES_JSON.filter((game) => { return game.pledged < game.goal;});
...
const listofGames = GAMES_JSON.filter((game) => { return game.pledged < game.goal;});
const lenU = listofGames.length;
console.log(lenU);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of ${totalAmount.toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${lenU == 1 ? "1 game" : lenU + " games"} remain unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let addedInfo= document.createElement("p");
addedInfo.textContent = displayStr;
descriptionContainer.append(addedInfo);
/************************************************************************************
...
// Bonus Challenge: I added a night mode toggle button 

function nightMode() {
    document.body.classList.toggle('night-mode');
}

const switchNight = document.getElementById("nightswitch-btn");

switchNight.addEventListener('click', nightMode);
